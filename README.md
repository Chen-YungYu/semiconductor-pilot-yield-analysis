...
## 📝 專案簡介
本專案專為半導體試產階段（Pilot Run）設計，打造互動分析 Data App。系統將複雜的製程數據進行自動化清洗與統計運算流程；導入 SPC 管制規則進行統計變異監控，結合 Box Plot 與無母數統計檢定科學化評估機台機差，並透過全子集合迴歸模型提供科學化的指標數據，輔助工程師進行關鍵製程參數的篩選與決策，快速定位影響良率的潛在因子。

## ✨ 系統核心分析功能說明 

本系統主要分為三大分析功能部份，透過統計學方法，評估機台機差，並篩選出對良率具顯著影響的關鍵製程參數（潛在因子）： 
